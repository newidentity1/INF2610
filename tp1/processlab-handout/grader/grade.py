...
PRINT_ORDER = ['q1', 'q2.1', 'q2.2', 'q2.3', 'q2.4']
...
def handleReportLine(reportLine):
    # Little trick here to ignore second execution of "./processlab 2" in the grading script
...
def handleQ1LtraceOutput(lines):
    rootPid = 0
    questionPid = 0
    writeCalled = False
    writePending = False
                ...
                if spl2[1].startswith('printf("') and spl2[1].find(' (printed using printf)') != -1:
                    if spl2[1].find('unfinished') != -1:
                        ...
                        if int(spl[-1]) > 0 and not writeCalled:
                            SCORES['q1']['printfBeforeWrite']['granted'] = True
                    continue
...
def grade(gradeLogPath, paramsFilePath, q1LtraceLogPath, part2StraceLogPath, part2LtraceLogPath, mode):
    ...
    with open(q1LtraceLogPath, "r") as q1LtraceLogFile:
        lines = q1LtraceLogFile.readlines()
        handleQ1LtraceOutput(lines)

    print("--------------------")
    print("Test Lab: Autograder")
    print("--------------------")
    print("")
...
if __name__ == '__main__':
    grade(sys.argv[1], sys.argv[2], sys.argv[3], sys.argv[4], sys.argv[5], sys.argv[6])
